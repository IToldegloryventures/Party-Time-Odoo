...
class CrmLead(models.Model):
    """Inherit CRM Lead to trigger dashboard KPI updates."""
    _inherit = "crm.lead"
    
    def write(self, vals):
        """Override write to trigger dashboard KPI update when leads change."""
        result = super().write(vals)
        
        # Trigger dashboard widget KPI update if relevant fields changed
        if any(field in vals for field in ['type', 'stage_id', 'user_id']):
    ...
    def create(self, vals_list):
        """Override create to trigger dashboard KPI update when leads are created."""
