...
class SaleOrder(models.Model):
    """Inherit Sale Order to trigger dashboard KPI updates."""
    _inherit = "sale.order"
    
    def write(self, vals):
        """Override write to trigger dashboard KPI update when orders change."""
        result = super().write(vals)
        
        # Trigger dashboard widget KPI update if relevant fields changed
        if any(field in vals for field in ['state', 'amount_total', 'user_id']):
    ...
    def create(self, vals_list):
        """Override create to trigger dashboard KPI update when orders are created."""
